...
- (NSDictionary *)dictionaryWithSystemAccount:(ACAccount *)systemAccount remoteAccount:(NSDictionary *)remoteAccount accessToken:(NSDictionary *)accessToken {
    ...
    //Banner Image
    NSString *banner = remoteAccount[@"profile_banner_url"];
    //    banner = [banner stringByAppendingString:@"/mobile_retina"];
    if (!banner) {
        banner = @"No Image";
    }
    else if ([[UIScreen mainScreen] respondsToSelector:@selector(displayLinkWithTarget:selector:)] && ([UIScreen mainScreen].scale == 2.0)) {
        banner = [banner stringByAppendingString:@"/mobile_retina"];
    } else {
        banner = [banner stringByAppendingString:@"/mobile"];
    }
    ...
    //Followers
    NSString *followers = remoteAccount[@"followers_count"];
    
    //Following
    NSString *following = remoteAccount[@"friends_count"];
